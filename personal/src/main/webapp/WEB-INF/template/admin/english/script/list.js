...
function initData(){
	...
	$.post(top.basePath+"english/list.do",formdata,function(result){
		if(result){
			if(result.flag){
				...
				for(var i=0;i<objs.length;i++){
					...
					$("#tr_"+objs[i].id+" td:eq(1)").click(function(){
						typeId=$(this).parent().attr("id").slice(3);
						openDialog(700,500,"../../admin/english/view.html","dialog_div","dialog_frame");
					});
				}
				window.using("pagination",function(){
					$("#pageBar").pagination({total:result.data.recordCount,pageSize:1000,showPageList:false});
				});
...
function edit(id){
	typeId=id;
	openDialog(900,500,"../../admin/english/add.html","dialog_div","dialog_frame");
}
function del(id,isSingle){
	...
	if(window.confirm("你确定要做傻事吗？")){
		$.post(top.basePath+"english/delete.do",{ids:idArray.join(",")},function(result){
			if(result){
				if(result.flag){
					alert("好吧！你成功做了一件傻事了！");
					initData();
